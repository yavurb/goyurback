...
func TestUpdatePost(t *testing.T) {
	...
	t.Run("it should return the updated post", func(t *testing.T) {
		postIn := map[string]any{
			"status": "published",
		}
		want := map[string]any{
		...
		req := httptest.NewRequest(http.MethodPatch, "/posts/:id", strings.NewReader(string(jsonBytes)))
		rec := httptest.NewRecorder()
		c := e.NewContext(req, rec)
		...
		uc := &mocks.MockPostsUsecase{}
		uc.UpdateFn = func(ctx context.Context, id string, title, author, slug, description, content *string, status *domain.Status) (*domain.Post, error) {
			if id != "po_12345" {
				return nil, domain.ErrPostNotFound
			}
		...
		err = h.updatePost(c)
		if err != nil {
			t.Errorf("Expected no error updating post. Got: %v", err)
		}

		if rec.Code != http.StatusOK {
			t.Errorf("Expected http status code to be %d. Got %d", http.StatusOK, rec.Code)
		}
		...
		if !testhelpers.CompareMaps(want, got) {
			t.Errorf("Mismatch updating post:\n%s", cmp.Diff(want, got))
		}
	})
	...
	t.Run("It should return a not found error", func(t *testing.T) {
		...
		err = h.updatePost(c)
		if err == nil {
			t.Error("Expected error updating post. Got nil")
		}
